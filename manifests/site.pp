...
node /^ganeti10(09|2[3456789]|3[0-9]|4[0-9]|5[0-2])\.eqiad\./ {
    role(ganeti)
}

# Old servers, to be decommed
node /^ganeti10(1[2678]|2[02])\.eqiad\./ {
    role(insetup::infrastructure_foundations)
}
...
node /^kafka-main100[4-8]\.eqiad\./ {
    role(kafka::main)
}

node /^kafka-main10(09|10)\.eqiad\./ {
    role(insetup::serviceops)
}

node /^kafka-main10(0[1-3])\.eqiad\./ {
    role(insetup::serviceops)
}
...
node /^kubernetes10(19|[2-5][0-9]|6[0-2]|)\.eqiad\./ {
...
node /^wikikube-worker10(0[1-9]|1[0-47-9]|2[0-9]|3[0-3])\.eqiad\./ {
    role(kubernetes::worker)
}
