...
node /^db2(136|140|147|172|179|206|210|219|236|237|240)\.codfw\./ {
    role(mariadb::core)
}
...
node /^db2(123|157|171|178|192|211|213|223)\.codfw\./ {
    role(mariadb::core)
}
...
node /^db22(26|28|31|32|33|34)\.codfw\./ {
    role(insetup::data_persistence)
}
...
node /^ganeti10(09|1[89]|2[023456789]|3[0-9]|4[0-9]|5[0-2])\.eqiad\./ {
    role(ganeti)
}
...
# Old servers, to be decommed
node /^ganeti20(1[567])\.codfw\./ {
    role(insetup::infrastructure_foundations)
}

node /^ganeti20(1[8-9]|2[0-9]|3[01256789]|4[0-4])\.codfw\./ {
    role(ganeti)
}
...
node /^kafka-main100[1-6]\.eqiad\./ {
    role(kafka::main)
}

node /^kafka-main10(0[7-9]|10)\.eqiad\./ {
...
node /^kubernetes10(0[56]|1[5-9]|[2-5][0-9]|6[0-2]|)\.eqiad\./ {
    role(kubernetes::worker)
}

node /^kubernetes20(0[5-6]|1[1-7]|2[1-4]|3[26-9]|4[0-9]|5[0-368])\.codfw\./ {
    role(kubernetes::worker)
}
...
node /^wikikube-worker1(2[4-9][0-9]|30[0-9]|31[0-2])\.eqiad\./ {
    role(kubernetes::worker)
}

node /^wikikube-worker20(0[1-47-9]|[1-9][0-9])\.codfw\./ {
    role(kubernetes::worker)
}
...
# T376307
node /^wikikube-worker13(1[3-9]|2[0-7])\.eqiad\./ {
    role(insetup::serviceops)
}
